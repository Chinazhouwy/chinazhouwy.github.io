...
> 本文默认你已经阅读了[设计模式入门：从 if、for 和 new 开始理解常见模式](./design-patterns-essence.md)。基础篇为了降低门槛做了一些简化，本文补充严格的定义、边界和框架实现。
...
**核心**：父类定义流程，子类决定具体产品。符合开闭原则——新增产品只需新增子类。
...
这是继承式模板方法的框架案例。`BaseExecutor` 定义了 SQL 执行的骨架（缓存检查、SQL 生成、执行、结果处理），子类各自实现具体的执行策略。
...
|---|---|---|---|---|
| 范围 | 进程内 | 进程内 | 进程内（默认） | 跨进程/跨服务 |
| 同步/异步 | 通常同步 | 可同步可异步 | 默认同步，可 @Async | 异步 |
| 持久化 | 无 | 无 | 无（默认） | 持久化 |
| 重试 | 无 | 通常无 | 无（默认） | 内置 |
| 顺序保证 | 取决于实现 | 取决于实现 | 取决于实现 | 通常支持分区有序 |
...
如果你需要入门视角，可以回看：[设计模式入门：从 if、for 和 new 开始理解常见模式](./design-patterns-essence.md)
