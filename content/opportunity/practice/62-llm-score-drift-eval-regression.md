...
> R0（2026-07-17）：不会，哈哈。
>
> R1（2026-07-22）：Eval 就是评估。方法一是找另一个 Agent 来评估（LLM-as-Judge），二是人工评估——准备预先的材料和要点，看回答有没有覆盖到。大概就这两种。方向正确，但未展开固定 Case 库、重复运行、双层门禁等核心机制。
...
- 正确理解 Eval = Evaluation
- 提到了 Agent 评估和人工评估两种方式
- 未涉及固定 Case 库、重复运行统计波动、绝对/相对门禁等回归机制
