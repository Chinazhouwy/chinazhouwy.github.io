...
### 用户回答
- LangGraph = 节点状态流转，有明确的下一步方向
- ReAct Loop = 大循环，每次根据模型反馈走下一步
- LangGraph 有明确方向，ReAct 没有
- LangGraph 第一个节点往往是意图识别

### 扣分点
1. 意图识别不是所有场景的第一步（-1）
2. 缺少 State/Node/Edge/Conditional Edge 四个核心概念（-1）
3. ReAct 能力取决于运行时，不是天生不支持（-1）

### 回顾收获
- LangGraph = 流程图（节点+边+条件），ReAct = 大循环（思考→行动→观察）
- 条件边可以由代码决定，也可以由模型动态路由
- 意图识别不是所有场景的第一步
- 选型依据：需要状态持久化/分支/人工介入 → LangGraph，简单任务 → ReAct
