...
function renderQuestions() {
  const items = state.query
    ? state.articles.filter((a) => canShowInSection(a) && matchesSearch(a))
    : filteredArticles("questions", { includePrivate: true });
  const groups = groupBy(items, "topic");
  const requestedTopic = new URLSearchParams(location.hash.split("?")[1] || "").get("topic");
  const entries = Object.entries(groups).sort((a, b) => b[1].length - a[1].length);
  ui.app.innerHTML = `
    <section class="directory-page">
      ${sectionIntro("ABILITY REVIEW", "能力复盘", "按技术方向整理具体题目、场景题、系统设计题和待复盘项。", items.length)}
...
function renderCompanies() {
  const items = filteredArticles("companies", { includePrivate: true });
...
function renderInterviews() {
  // Include both "interviews" section articles and practice (questions) articles that have scores
  const interviewSection = filteredArticles("interviews", { includePrivate: true });
  const practiceArticles = filteredArticles("questions", { includePrivate: true })
    .filter((a) => a.score !== null || a.questionNumber !== null);
  // Merge and deduplicate by path
  const seen = new Set();
  const items = [...interviewSection, ...practiceArticles].filter((a) => {
...
function renderPlans() {
  const items = filteredArticles("plans", { includePrivate: true });
  ui.app.innerHTML = `
...
const BUILD_VERSION = "20260706-4";
